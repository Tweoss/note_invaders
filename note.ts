...
export class Note {
  ...
  move(delta: number) {
    ...
    const on = (v) =>
      v < transition[0] ? 0 : Math.exp(-1 / (v - transition[0]));

    const desired_volume =
      on(this.y) / (on(this.y) + on(transition[1] - this.y));
    this.audio_node.gain.exponentialRampToValueAtTime(
      desired_volume,
...
// Take A = 440 as 0.
function note_to_frequency(note: number): number {
  return 440 * Math.pow(2, (1 / 12) * note);
}
export function round_frequency_to_note(frequency: number): number {
