...
const skip_while = (array, predicate) => {
    const index = array.findIndex((e) => !predicate(e));
    return array.slice(Math.max(index, 0));
};
const zip = (a, b) => a.map((e, i) => [e, b[i]]);
const max_index_by = (array, left_greater) => array.reduce((acc, e, i) => (left_greater(array[acc], e) ? acc : i), 0);
// See https://github.com/cwilso/PitchDetect/blob/main/js/pitchdetect.js
export function find_pitch(buffer, sample_rate) {
    let root_mean_square = Math.sqrt(sum(buffer.map((v) => v * v)) / buffer.length);
    // Ignore silence.
    ...
    buffer = skip_while(buffer, is_nonzero);
    ...
    const offset = zip(counts, counts.slice(1)).findIndex(([el, succ]) => el <= succ);
    let best_index = max_index_by(counts.slice(offset), (a, b) => a > b) + offset;
    // Corrections?
    const a = (counts[best_index - 1] + counts[best_index + 1] - 2 * counts[best_index]) /
        2;
